...
def quality_focal_loss(pred, target, beta=2.0):
    ...
    score = score.new_ones(score.shape)   # zx modified
    ...
    ratio = label.bincount()[:bg_class_ind] / label.bincount()[:bg_class_ind].max().clamp_min(1e-8) # zx modified
    class_weight0 = 2.0 - ratio # zx modified
    class_weight1 = 1.5 + 0.5 * torch.cos(torch.pi * ratio)  # zx modified
    class_weight = torch.where(ratio > 0.0, torch.where(ratio<=0.5, class_weight0, class_weight1), torch.tensor(1.0).to(ratio.device))
    loss[pos, pos_label] = F.binary_cross_entropy_with_logits(
        pred[pos, pos_label], score[pos],
        weight=class_weight[pos_label],   # zx modified
