...
class DetLocalVisualizer(Visualizer):
    ...
    def add_datasample(
        ...
        if image_extra is not None:
            image_extra = image_extra.clip(0, 255).astype(np.uint8)
        ...
        if draw_gt and data_sample is not None:
            ...
            if 'gt_instances' in data_sample:
                gt_img_data = self._draw_instances(image,
                                                   ...
                                                   classes, palette)
                if image_extra is not None:
                    gt_img_data_extra = self._draw_instances(image_extra,
                                                    data_sample.gt_instances,
                                                    classes, palette)
        ...
        if draw_pred and data_sample is not None:
            pred_img_data = image
            pred_vis_img_data = mmcv.imread(data_sample.img_path.replace('infrared', 'visible'), channel_order='rgb').clip(0, 255).astype(np.uint8)   # zx modified
            if 'pred_instances' in data_sample:
                ...
                pred_img_data = self._draw_instances(image, pred_instances,
                                                     classes, palette)
                pred_vis_img_data = self._draw_instances(pred_vis_img_data, pred_instances, # zx modified
                                                     classes, palette)
        ...
        if gt_img_data is not None and pred_img_data is not None:
            ...
            drawn_img = np.concatenate((gt_img_data, pred_img_data, pred_vis_img_data), axis=1) # zx modified
        elif gt_img_data is not None:
            drawn_img = gt_img_data
            if image_extra is not None:
                drawn_img = np.hstack([gt_img_data, gt_img_data_extra])
